...
public final class GameState extends State implements TouchListener{
	...
	public static void resetValues(){
		GameState.getInstance().countPlayer1 = 0;
		GameState.getInstance().countPlayer2 = 0;
		GameState.getInstance().paddle1.setPosition(225, 80);
		GameState.getInstance().paddle2.setPosition(225, 610);
	}
	...
	public void update(float dt) {
		...
		if(countPlayer1==4){
			getGame().popState();
			GameOver.setWinner(1);
			getGame().pushState(GameOver.getInstance());
		}
		if(countPlayer2==4){
			getGame().popState();
			GameOver.setWinner(2);
			getGame().pushState(GameOver.getInstance());
		}
